import {
    ifElseDoNothing,
    pipe,
    provideDayStateProvider, tap, createNextWeekDay,
} from './utilities';
import {
    generator0,
    IIntake,
...
const isToday = (weekDays: IWeekDays, day: string): boolean => {
    return weekDays[day];
}
...
export function calculateFp(intakes: IIntake[], stock: number, weekDays: IWeekDays): string | Date {
    const date = new Date();
    const getCalculatedDate = getDate(date);
    ...
    const isSubtractingFirstDay = isToday(weekDays, currentDay);
